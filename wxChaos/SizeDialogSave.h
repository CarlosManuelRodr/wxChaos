...
public:
	SaveProgressDiag(Fractal* targetFractal, wxWindow* parent, bool _saveProgressAvailable = true, wxWindowID id = wxID_ANY, const wxString& title = wxT("Saving..."),
						const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxSize( 366,127 ), long style = wxDEFAULT_DIALOG_STYLE );
	~SaveProgressDiag();
...
public:
	...
	SizeDialogSave(FractalCanvas *mFCanvas, string rutaArchivo, int ext, FRACTAL_TYPE type, Fractal *target, wxWindow* parent, string scriptPath = "",
					wxWindowID id = wxID_ANY, const wxString& title = wxT("Select size"), const wxPoint& pos = wxDefaultPosition,
					const wxSize& size = wxSize(283,201), long style = wxDEFAULT_DIALOG_STYLE);
	~SizeDialogSave();
