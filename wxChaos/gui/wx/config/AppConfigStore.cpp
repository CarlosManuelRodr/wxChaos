...
namespace
{
    ...
    bool HasIniSections(const std::string& filename)
    {
        std::ifstream file(filename.c_str());
        std::string line;
    ...
    std::map<std::string, std::string> ReadLegacyConfig(const std::string& filename)
    {
    ...
    bool ReadBool(const std::map<std::string, std::string>& values, const std::string& key, bool defaultValue)
    {
    ...
    int ReadInt(const std::map<std::string, std::string>& values, const std::string& key, int defaultValue)
    {
        const auto it = values.find(key);
        if (it == values.end())
            return defaultValue;
        ...
        try
        {
            ...
            const int value = std::stoi(Trim(it->second), &parsedChars);
            return parsedChars == Trim(it->second).size() ? value : defaultValue;
        }
        catch (...)
        {
            return defaultValue;
    ...
    std::string ReadString(const std::map<std::string, std::string>& values, const std::string& key, const std::string& defaultValue)
    {
        const auto it = values.find(key);
        return it == values.end() ? defaultValue : it->second;
    }

    const std::map<std::string, FractalType>& FractalTypes()
    {
    ...
    FractalType FractalTypeFromString(const std::string& value, const FractalType defaultValue)
    {
        const auto it = FractalTypes().find(value);
        return it == FractalTypes().end() ? defaultValue : it->second;
    }

    std::string FractalTypeToString(FractalType type)
    {
    ...
    AppConfig LoadLegacyConfig(const std::string& filename)
    {
        AppConfig config;
