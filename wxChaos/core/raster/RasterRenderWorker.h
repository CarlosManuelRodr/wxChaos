...
* @brief Base class for raster fractal workers that render rectangular pixel regions.
*
* RasterRenderWorker stores the shared render options and output maps for one pixel
...
*/
class RasterRenderWorker : public RenderWorker
{
protected:
    ...
    int _x;
    int _y;
    mutable unsigned int _threadProgress;
    ...
    void RenderFromPoint(TracePoint tracePoint, double (RasterRenderWorker::*colorPoint)(const Point&) const, MeasurePoint measure);
...
public:
    RasterRenderWorker();
    ~RasterRenderWorker() override = default;
    ...
    Options GetOptions();
    void SetOutputRenderingMaps(bool** outSetMap, double** outColorMap);
...
template<class PixelRenderer> void RasterRenderWorker::RenderPixels(PixelRenderer pixelRenderer)
{
...
template<class PixelRenderer> void RasterRenderWorker::RenderPixelsPrecise(PixelRenderer pixelRenderer)
{
...
template<class PixelRenderer> void RasterRenderWorker::RenderPixelsByPrecision(PixelRenderer pixelRenderer)
{
...
template<class TracePoint, class MeasurePoint>
void RasterRenderWorker::RenderFromPoint(TracePoint tracePoint, double (RasterRenderWorker::*colorPoint)(const Point&) const, MeasurePoint measure)
{
...
template<class TracePoint>
void RasterRenderWorker::EscapeTimeRender(TracePoint tracePoint)
{
    if (_myOpt.orbitTrapMode)
    {
        ...
        RenderFromPoint(tracePoint, &RasterRenderWorker::EscapeTimeColor, measure);
        return;
    }
...
template<class TracePoint>
void RasterRenderWorker::GaussianIntRender(TracePoint tracePoint)
{
    if (_myOpt.orbitTrapMode)
    {
        ...
        RenderFromPoint(tracePoint, &RasterRenderWorker::GaussianIntegerColor, measure);
        return;
    }
...
template<class TracePoint>
void RasterRenderWorker::EscapeAngleRender(TracePoint tracePoint)
{
    const auto measure = [](Point&, PointTraceEvent, unsigned int, double, double, double, double, double, bool) {};
    RenderFromPoint(tracePoint, &RasterRenderWorker::EscapeAngleColor, measure);
}

template<class TracePoint>
void RasterRenderWorker::TriangleInequalityRender(TracePoint tracePoint)
{
    ...
    RenderFromPoint(tracePoint, &RasterRenderWorker::TriangleInequalityColor, measure);
}
