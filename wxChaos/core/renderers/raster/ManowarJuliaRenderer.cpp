...
RasterRenderWorker::Point ManowarJuliaRenderer::TracePoint(const Real& pixelRe, const Real& pixelIm, MeasurePoint measure) const
{
    Point point;
    ...
    for (unsigned n = 0; n < _maxIterations; n++)
    {
        ...
        zIm = Real(2) * zRe * zIm + Real(_kImaginary) + manIm;
        zRe = zRe2 - zIm2 + Real(_kReal) + manRe;
        manRe = tempRe;
        manIm = tempIm;
