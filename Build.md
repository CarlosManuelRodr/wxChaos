...
### CMake cannot find SymEngine

SymEngine may be installed in the project-local manifest directory while CMake
is searching the classic vcpkg install tree. Set one of these:
...
### CMake says CMake < 3.5 compatibility was removed

This comes from SymEngine's exported CMake config when using CMake 4. Keep the
`CMAKE_POLICY_VERSION_MINIMUM` workaround in `CMakeLists.txt`.

### `cmake` is not recognized

Plain PowerShell may not have CMake on `PATH`. Use CLion's bundled CMake by
absolute path, or configure/build through CLion.

### MSVC cannot find standard headers such as `vector` or `complex`

The build was launched outside the Visual Studio developer environment. Re-run
the Ninja command through `VsDevCmd.bat`.

### vcpkg writes fail during configure

CMake can invoke vcpkg during configure. Make sure the selected vcpkg directory
is writable and that no other vcpkg process is holding the manifest lock.

### Stale incremental build errors

If link errors mention both old and renamed symbols, clean and rebuild:

```powershell
cmd /c "call ""C:\Program Files\Microsoft Visual Studio\18\Community\Common7\Tools\VsDevCmd.bat"" -arch=x64 -host_arch=x64 && ninja -C cmake-build-debug clean && ninja -C cmake-build-debug wxChaos"
```
